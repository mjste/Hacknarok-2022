...
import { deadlineArray, NextDeadline, Deadline, compare } from '../../components/NextDeadline'
			...
			<View style={styles.DeadlinesStyle}>
				{deadlineArray.sort(compare).map((item, index) => {
					if (item.date.getDate() != prevDate) {
						...
						return [<DayHeader date={item.date} key={2 * index} />, <NextDeadline index={index} key={2 * index + 1} />]
					}
	...
	return (
		<View>
			<Text style={styles.DayHeaderStyle}>
				Day: {props.date.getDate()}.{props.date.getMonth()}.{props.date.getFullYear()}
			</Text>
		</View>
	)
}
